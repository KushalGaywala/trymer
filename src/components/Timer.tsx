...
import { resolveBackgroundImageUrl, buildComponentStyle, COMPONENT_IDS, type ComponentId } from '@/lib/app-settings';
...
export function Timer() {
  const {
    ...
    moveComponent, reorderWithinSlot, updateComponent, setSlotDirection,
  ...
  const tick = useCallback(() => {
    const sl = settings.labels.study || 'Timer';
    ...
    if (timerSwitch) {
      if (secondCount < 59 && minuteCount < studyTime) {
        setTimerContext(ctx(sl, sessionCount, totalSessions)); setSecondCount((s) => s + 1);
      ...
      if (secondCount < 59 && minuteCount < breakTime) {
        ...
        setTimerContext(ctx(sl, sessionCount, totalSessions)); playAlertAudio();
      } else { setSecondCount((s) => s + 1); }
    }
  ...
  // Helper: build a ComponentEntry for an id given its node
  ...
  const allEntries: Record<ComponentId, ComponentEntry> = {
    ...
    timerStudyInput: makeEntry('timerStudyInput',
      <TimerInputBlock inputType="study" value={studyTime} isRunning={isRunning}
        onChange={setStudyTime} label={settings.labels.study || 'Study'}
        size={comps.timerStudyInput.size} align={comps.timerStudyInput.align}
      />),
  ...
  // Build slot map: slot → ComponentEntry[] sorted by order
  const slotMap: Record<string, ComponentEntry[]> = {};
  for (const id of COMPONENT_IDS) {
    const slot = comps[id].slot;
    if (!slot) continue;
    if (!slotMap[slot]) slotMap[slot] = [];
    slotMap[slot].push(allEntries[id]);
  }
  for (const slot in slotMap) {
    slotMap[slot].sort((a, b) => comps[a.id].order - comps[b.id].order);
  }
    ...
    <div className={cn(
      ...
      <div className="relative z-[2] flex min-h-screen flex-col">
        ...
        <AppSettingsSheet
          ...
          setSlotDirection={setSlotDirection}
        ...
        <TimerGrid
          ...
          slotMap={slotMap}
          slotDirections={settings.slotDirections}
          onMove={moveComponent}
          onReorder={reorderWithinSlot}
        />
      </div>
    </div>
  );
